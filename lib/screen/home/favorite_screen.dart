...
class _FavoriteScreenState extends State<FavoriteScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Favorite',style: TextStyle(color: Colors.deepOrange,fontFamily: 'MyFont')),
      ...
      body: Column(
        children: List.generate(popFav.length, (index){
          return ListTile(
            title: Text(popFav[index].title,style: TextStyle(fontFamily: 'MyFont',fontWeight: FontWeight.bold),),
            leading: Image.network(popFav[index].image),
            subtitle: Text(popFav[index].price,style: TextStyle(fontFamily: 'MyFont',color: Colors.deepOrange),),
            trailing: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: (liked[popFav[index].id-1])?Colors.pink.withOpacity(0.2):Colors.grey.withOpacity(0.2),
                ),
                child: Icon(Icons.favorite,size: widthOrHeight(context, choice: 1)*0.04,color: (liked[popFav[index].id-1])?Colors.pink:Colors.grey,),
              ),
            ),
          );
        }
